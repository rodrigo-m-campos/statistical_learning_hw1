...
```
We remove certain outliers (Who goes to the doctor 40 times a year and still claims they are fine?)
```{r}
...
Check all the variable relations (primarily with mdu)
```{r}
pairs(data)
```

Doesn't give many ideas. Maybe visually we can see something.
```{r}
...
mse3
```
Since we removed the outliers, these results are not that bad.
...
We can probably try just taking the first three components (even if it isn't such a good representation).
...
How much does each variable contribute? (another way of seeing it)
```{r}
...
####Fourth PC
```{r}
plot_data_4 = data.frame(PC4 = pca$x[,4], Target = data$mdu)
ggplot(plot_data_4, aes(x = PC4, y = Target)) +
...
We stick to 3 factors for simplicity.
```{r}
par(mfrow=c(3,1))
...
We plot using the first two PC's and also checking health, in order to get an idea.
```{r}
fviz_pca_ind(pca, geom = "point", habillage = data$health)
```

Seems like two clusters would make sense (or maybe even four).

We first try with two clusters.
```{r}
...
fviz_cluster(fit.pam_2, geom = "point", data = data_num_scaled, main = "Cluster with PAM (4)")
```
...
We now use hierarchical clustering.
```{r}
